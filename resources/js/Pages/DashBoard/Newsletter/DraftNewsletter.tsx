...
const DraftNewsletter = ({
  ...
  const [currentPreviewNewsletter, setCurrentPreviewNewsletter] =
    useState<NewsletterWithStatus | null>(null)
        ...
        {draftNewsletters.map((newsletter, index) => {
          ...
          return (
            <Card
              key={index}
              className={cn(
                'py-3 hover:bg-muted group/draftcard',
                (currentPreviewNewsletter
                  ? currentPreviewNewsletter.id === newsletter.id
            ...
            >
              <CardContent
                className={
                  'flex flex-col justify-center items-stretch gap-1 pr-2.5'
                }
              >
                <div
                  className={'flex flex-row justify-between items-center gap-2'}
                >
                  <p className={'font-semibold cursor-default truncate'}>
                    {newsletter.subject}
                  </p>
                  <div
                    className={
                      'cursor-pointer hidden group-hover/draftcard:flex self-start hover:bg-muted'
                    }
                    onClick={(e) => {
                      e.stopPropagation()
                ...
                <p
                  className={
                    'text-sm text-muted-foreground text-wrap cursor-default'
                  }
                >
                  {htmlToText(newsletter.content).slice(0, displayCharLimit)}
                  {'...'}
                </p>
              </CardContent>
            </Card>
      ...
      <ResizablePanel>
        {isNewsletterSelected && (
          ...
          <ComposeNewsletterTextEditor
            newsletter={currentPreviewNewsletter}
            onSend={() => {
        ...
        {!isNewsletterSelected && (
          <div
            className={
              'w-full h-full flex justify-center items-center text-muted-foreground'
            }
          >
            No draft newsletter selected
          </div>
        )}
      </ResizablePanel>
    </ResizablePanelGroup>
...
DraftNewsletter.layout = (page: React.ReactNode) => (
  <NewDashBoardLayout activeSubPageName="DraftNewsletter">
    {page}
  </NewDashBoardLayout>
)
