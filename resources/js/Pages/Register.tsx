...
const emailLocalPartRegex = /^([-!#-'*+\/-9=?A-Z^-~]+(\.[-!#-'*+\/-9=?A-Z^-~]+)*|"([]!#-[^-~ \t]|(\\[\t -~]))+")$/
...
const passwordFormZod = z.object({
    email: z.string().email().optional().nullable().transform(val => undefined),
...
export default function RegisterPage(props: RegisterPageProps) {
    ...
    const [userRegistrationData, setUserRegistrationData] = useState<UserRegistrationData>(defaultUserRegistrationData);
    const [finalSubmit, setFinalSubmit] = useState<boolean>(false);
    const [profilePictureType, setProfilePictureType] = useState<string>("image/png");
    ...
    useEffect(() => {
        if (userRegistrationData.email !== "") {
            const name = emailNameRegex.exec(userRegistrationData.email)?.groups?.['local_part'] ?? "";
    ...
    const [isUploadImageDialogOpen, setIsUploadImageDialogOpen] = useState<boolean>(false);
        ...
        load(async () => {
            try {
                ...
                if (response.status === 200 && response.data['status'] === 'ok') {
                    setUserRegistrationData(previousValue => ({...previousValue, email: data.email}))
                    carouselApi?.scrollNext()
                }

            } catch (error: any | AxiosError) {
                if (error instanceof AxiosError && error?.response?.status === 403 && error?.response.data['status'] === 'failed') {
    ...
    async function handleProfilePictureFileSelection(e: ChangeEvent<HTMLInputElement>) {
        const imageFile = e.target.files?.[0];
        const imageFileExtensionName = imageFileExtensionrRegex.exec(imageFile?.name ?? "")?.groups?.['file_extension']
        if (imageFile) {
    ...
    return (
        <div className={"w-full h-screen flex flex-col justify-center items-center gap-6"}>
            <LogoText/>
            <Carousel setApi={setCarouselApi} opts={{align: "start", watchDrag: false}}
                      className={"w-[80%] sm:w-[55%] md:w-[45%] lg:w-[35%] xl:w-[25%] 2xl:w-[20%]"}>
                <CarouselContent className={"px-2"}>
                    <CarouselItem>
                        <Form {...emailForm} >
                            <form id={emailFormId} className={"flex flex-col gap-5 items-stretch"}
                                ...
                                <Button disabled={isLoading} isLoading={isLoading} type={"submit"}
                                        form={emailFormId}>Next</Button>
                            </form>
                        </Form>
                    </CarouselItem>
                    <CarouselItem className={"items-stretch"}>
                        <Form {...passwordForm}>
                            <form id={passwordFormId} className={"flex flex-col gap-5 items-stretch"}
                                  onSubmit={passwordForm.handleSubmit(handlePasswordFormSubmission)}>
                                ...
                                <FormField shouldUnregister disabled name={"email"} render={({field}) => {
                                    useEffect(() => {
                                ...
                                <div className={"flex flex-row gap-3 self-stretch justify-between"}>
                                    <Button disabled={isLoading} isLoading={isLoading} type={"button"}
                                            variant={"secondary"}
                                            onClick={() => carouselApi?.scrollPrev()}>Back</Button>
                                    <Button disabled={isLoading} isLoading={isLoading} type={"submit"}
                                            form={passwordFormId}>Next</Button>
                                </div>
                            </form>
                        </Form>
                    </CarouselItem>
                    <CarouselItem className={"pr-1 flex flex-col items-center"}>
                        <Form {...profileForm} >
                            <form className={"flex flex-col gap-3"} id={profileFormId}
                                           ...
                                           render={({field}) => {
                                               return (
                                                   <FormItem className={"flex flex-col items-center gap-3"}>
                                                       <Avatar
                                                           className={"w-32 h-32 border-2 border-primary shadow-lg"}>
                                                           <AvatarImage src={field.value ?? "/default_avatar.png"}
                                                                        alt={"User Profile Picture"}/>
                                                       </Avatar>
                                                       <Dialog open={isUploadImageDialogOpen}
                                                           ...
                                                           <DialogContent>
                                                               <UploadImage disableFromUrl
                                                                            onImageFileSelectionChange={handleProfilePictureFileSelection}
                                ...
                                <FormField name={"name"} render={({field}) => {
                                    return (
                                        <FormItem>
                                            ...
                                            <Input {...field} />
                                            <FormDescription>
                                                <p>This will be show up in your email newsletter `From` header. For
                                                    example: Kiethseng@mailixer.cc. Display name should conform to below
                                                    rule:</p>
                                                <ul className={"mt-3 list-decimal px-5"}>
                                                    <li>{"Uppercase and lowercase English letters (a-z, A-Z)"}</li>
                                                    <li>{"Digits (0-9)"}</li>
                                                    <li>{"Special characters: ! # $ % & ' * + - / = ? ^ _ ` { | } ~"}</li>
                                                    <li>{"Dot (.), but not as the first or last character and not consecutively (i.e., \"..\")"}</li>
                                                </ul>
                                            </FormDescription>
                                            <FormMessage/>
                                        </FormItem>
                                ...
                                <Button disabled={isLoading} isLoading={isLoading} type={"submit"} form={profileFormId}>Sign
                                    Up</Button>
                                <Button disabled={isLoading} isLoading={isLoading} type={"button"} variant={"ghost"}
                                        onClick={() => carouselApi?.scrollPrev()}>Back</Button>
                            </form>
                        </Form>
