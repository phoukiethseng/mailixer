...
const InlineStyledHeading = Heading.extend({
    renderHTML({node, HTMLAttributes}) {
        ...
        return [`h${level}`, mergeAttributes(HTMLAttributes, {style: withCommonCssProperties(appliedStyle)}), 0]
    }
});
export const EditorExtensionList = [StarterKit, Underline, Link, FixedSizeImage, InlineStyledParagraph, InlineStyledBold, InlineStyledHeading];
