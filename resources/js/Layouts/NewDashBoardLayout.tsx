...
function getPageGroupDescriptionFor(
  subPageName: DashBoardMenuItems
): DashBoardPageGroupDescription | null {
  let pageGroup = null
...
function getPageDescriptionFor(
  pageName: DashBoardMenuItems
): DashBoardPageDescription | null {
...
const NewDashBoardLayout = ({
  ...
  const profilePicture = useProfilePicture(
    inertiaPage.props.auth.user.profilePictureUrl
  )
    ...
    <div className={'flex flex-col w-full h-screen overflow-hidden'}>
      ...
      >
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant={'ghost'}
                className={
                  'flex flex-row justify-start items-center gap-2 px-2 py-1 rounded-md hover:bg-muted cursor-pointer '
                }
              >
                ...
                <span
                  className={
                    'text-muted-foreground text-sm font-semibold max-w-44 truncate'
                  }
                >
                  {userDisplayName}
                </span>
            ...
            <PopoverContent className={'flex flex-col gap-4'}>
              <div className={'flex flex-row justify-start items-center gap-4'}>
                ...
                <div className={'flex flex-col justify-start items-stretch'}>
                  <span
                    className={
                      'truncate max-w-44 text-sm text-foreground font-semibold'
                    }
                  >
                    {userDisplayName}
                  </span>
                  <span
                    className={
                      'truncate max-w-44 text-xs text-muted-foreground'
                    }
                  >
                    {userEmail}
                  </span>
        ...
        <nav className={'flex flex-row h-full gap-0'}>
          {currentPageGroup &&
            currentPageGroup.pages.map((page, index) => {
              ...
              return (
                ...
                >
                  <div
                    className={cn(
                      'flex flex-row justify-center items-center gap-3 px-5',
                  ...
                  {isActive && (
                    <span
                      className={'w-full bg-primary h-[3px] absolute bottom-0'}
                    />
                  )}
                </Link>
              )
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            ...
            <DropdownMenuItem
              onClick={() => router.visit(route('setting.account'))}
            >
              Account Setting
            </DropdownMenuItem>
          </DropdownMenuContent>
